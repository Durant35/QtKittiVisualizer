...
KittiDataset::KittiDataset(std::string dataset) :
        dataset_(dataset),
        number_of_frames_(0)
{
    if (!boost::filesystem::exists(KittiFileSystem::getPointCloudPath(dataset_, 0))) {
        std::cerr << "Error in KittiDataset: No point cloud was found at "
                  << (KittiFileSystem::getPointCloudPath(dataset_, 0)).string()
                  << std::endl;
        return;
    }
    if (!boost::filesystem::exists(KittiFileSystem::getTrackletsPath(dataset_))) {
        std::cerr << "Error in KittiDataset: No tracklets were found at "
                  << (KittiFileSystem::getTrackletsPath(dataset_)).string()
                  << std::endl;
        return;
    }
...
KittiPointCloud::Ptr KittiDataset::getPointCloud(int frameId)
{
    ...
    std::fstream file(KittiFileSystem::getPointCloudPath(dataset_, frameId).c_str(), std::ios::in | std::ios::binary);
    if (file.good()) {
...
{
    boost::filesystem::directory_iterator dit(KittiFileSystem::getPointCloudDir(dataset_));
...
{
    boost::filesystem::path trackletsPath = KittiFileSystem::getTrackletsPath(dataset_);
